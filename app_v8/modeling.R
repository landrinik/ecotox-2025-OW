...
compute_did_contrasts <- function(model, baseline_week, target_week, dose_val, is_ctrl, 
                                 scope = c("by_fiber", "pooled"), adj_method = "none") {
  scope <- match.arg(scope)
  
  # Step 1: EM means by treatment within fiber and week at chosen dose
  ...
  # Step 2: Within each fiber × week, compute Treated - Untreated (control)
  tc <- emmeans::contrast(emm_tw, method = "trt.vs.ctrl", ref = "untreated")
  
  # Keep only baseline and target weeks
  df_tc <- as.data.frame(tc) %>% 
    dplyr::filter(week %in% c(baseline_week, target_week))
  
  # Order rows to match c(baseline, target) within each fiber
  lvl_week <- c(baseline_week, target_week)
  df_tc <- df_tc %>% dplyr::arrange(fiber_type, factor(week, levels = lvl_week))
  
  if (scope == "by_fiber") {
    # Build per-fiber contrast: target - baseline
    lv_fiber <- unique(df_tc$fiber_type)
    out <- df_tc %>%
      dplyr::group_by(fiber_type) %>%
      dplyr::summarise(
        estimate = diff(estimate), # target - baseline
        SE = sqrt(sum(SE^2)), # conservative SE estimation
        .groups = "drop"
      ) %>%
      dplyr::mutate(
        t_stat = estimate / SE,
        p.value = 2 * pt(-abs(t_stat), df = Inf), # approximate
        contrast = paste0("DiD (", fiber_type, "): target - baseline")
      )
    ...
    pooled <- df_tc %>%
      dplyr::group_by(week) %>%
      dplyr::summarise(
        emmean = mean(estimate, na.rm = TRUE),
        SE = sqrt(mean(SE^2, na.rm = TRUE)),
        .groups = "drop"
      ) %>%
      dplyr::arrange(factor(week, levels = lvl_week))
    
    out <- tibble::tibble(
      contrast = "DiD (pooled): target - baseline",
      estimate = pooled$emmean[2] - pooled$emmean[1],
      SE = sqrt(sum(pooled$SE^2)),
      t_stat = estimate / SE,
      p.value = 2 * pt(-abs(t_stat), df = Inf)
    )
  }
  
  # Apply p-value adjustment
  if (adj_method != "none" && nrow(out) > 1) {
    out$p.value <- p.adjust(out$p.value, method = adj_method)
  }
